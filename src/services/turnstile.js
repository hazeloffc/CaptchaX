...
class BypassService {
  ...
  loadFakePage() {
    try {
      const candidates = [
        path.join(__dirname, "assets", "fakePage.html"),
        path.join(process.cwd(), "src", "assets", "fakePage.html"),
        path.join(process.cwd(), "assets", "fakePage.html"),
      ];
      let fakePagePath = candidates.find((p) => fs.existsSync(p)) || candidates[0];
